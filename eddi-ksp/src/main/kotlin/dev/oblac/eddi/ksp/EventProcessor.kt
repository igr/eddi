...
class EventProcessor(
    ...
    private fun writeSourceFile(
        ...
        val tagsForEventClass = tagImplementations
            .filter { (_, v) -> v.endsWith(".$className") }
            .map { (k, _) -> k }
        if (tagsForEventClass.size > 1) {
            error("Class $className has more than 1 tag implementations: $tagsForEventClass")
        }
        val tagClass = tagsForEventClass.firstOrNull()?.let { simpleClassNameOf(it) }
        ...
        writer.write(
            """
                ...
                |
                |/**
                | * Generated meta companion class for [$className].
                | */
            ...
            if (tagClass != null) {
                """
                |
                |@JvmName("fun$tagClass")
                |fun dev.oblac.eddi.EventEnvelope<$className>.tag(): $tagClass {
                |    return $tagClass(this.sequence)
                |}
                |
                """.trimMargin()
            } else { "" }
